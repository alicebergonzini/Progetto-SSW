...
export class ResultComponent implements OnInit {
  ...
  checkStatus() {
    this.status =
      this.book_result.utenteNol === undefined ? 'Disponibile' : 'Noleggiato';
  ...
  openNoleggia() {
    ...
    var div_result: HTMLDivElement = document.getElementById(
      this.id
    ) as HTMLDivElement;
  ...
  //metodo che fa una get per aggiornare l'archivio e poi fa una set con il libro selezionato noleggiato all'utente
  ...
  //metodo che fa una get per aggiornare l'archivio e poi fa una set con il libro selezionato privo di presatario (ovvero utenteNol=undefined)
  returnBook() {
    if (
      confirm('Sicuro di voler restituire ' + this.book_result.titolo + '?')
    ) {
      this.ls.getLibrary().subscribe({
        next: (x: AjaxResponse<any>) => {
          var booklist = JSON.parse(x.response);
        ...
        error: (err) =>
          console.error(
            'La richiesta ha dato un errore: ' + JSON.stringify(err)
          ),
      });
    }
  }
